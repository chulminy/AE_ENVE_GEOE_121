...
[~, I] = sort(data, 'descend');
ranked_data = team_data(I,:);

end
```
## M07055 Function: Alphabet Game (★) 
<img src="https://user-images.githubusercontent.com/104775472/181603484-b72c1e15-5afd-48bf-ab86-d9bcbe65096a.png" width="350">

Jesse and Chul Min create an alphabet number game. A goal is to score a 1 x 10 lowercase alphabet sequence. A player with a higher score wins this game. 
The score of 'a' is 50 and the score of each following letter gradually decreases by 2. For example, 'b' becomes 48, 'c' becomes 46 and 'z' becomes 0. 
The final score is found by summing the scores of all 10 letters in a given sequence. 

Your function will be named 'FindWin', with two inputs and three outputs:
```matlab
function [p1_score,p2_score,is_p1_win] = FindWin(p1_seq,p2_seq)
```
- p1_score: score of the input sequence 'p1_seq'
- p2_score: score of the input sequence 'p2_seq'
- is_p1_win: true if p1 score is higher than p2 score. Otherwise, false. 

**Solutions**

```matlab
alph_seq = 'abcdefghijklmnopqrstuvwxyz';
p1_seq = alph_seq(randi(26, 1, 10)); % 1x10 character vector 
p2_seq = alph_seq(randi(26, 1, 10)); % 1x10 character vector 

clearvars alph_seq

[p1_score,p2_score,is_p1_win] = FindWin(p1_seq,p2_seq);

% Write your function below

function [p1_score,p2_score,is_p1_win] = FindWin(p1_seq,p2_seq)

p1_scores = (p1_seq - 'a')*-2 + 50;
p1_score = sum(p1_scores);

p2_scores = (p2_seq - 'a')*-2 + 50;
...
## M07056 Function: Finding the Median of the Diagonal of a Matrix (★★)

Create a function named 'DiagMat' that has 2 inputs and 1 output:
```matlab
function med = DiagMat(mat, direc)
```
'mat' is a n x n matrix with random numbers. You will need to make the function to compute a median value from diagonal or antidiagonal values in 'mat'
When 'direc' is 'diag', a median value from diagonal values in 'mat' is computed and assigned to 'med'. 
When 'direc' is 'anti_diag', a median value from anti-diagonal values in 'mat' is computed and assigned to 'med'. 
For example, an input matrix is 'mat' below
```matlab
mat =
...
Then, 
```matlab
med = DiagMat(mat, 'diag') % Median of 37,21,and 44
...
or 
```matlab
med = DiagMat(mat, 'anti_diag') % Median of 25,21, and 81
...
**Solution**
```matlab
mat1 = randi([1 100],9,9); % 9x9 matrix with random numbers
med1 = DiagMat(mat1, 'diag');

mat2 = randi([1 100],17,17); % 17x17 matrix with random numbers
med2 = DiagMat(mat2, 'anti_diag');

function med = DiagMat(mat, direc)

if isequal(direc, 'diag')
    input = mat;
    diag_vec = diag(input);
    med = median(diag_vec);
elseif isequal(direc, 'anti_diag')
    input = flip(mat, 2);
    diag_vec = diag(input);
    med = median(diag_vec);
else
    error('incorrect direc value');
end
...
**Do not use the function 'rem' when solving this problem**
In this question you need to create two functions, 'MyRem1' and 'MyRem2'
(a) 'MyRem1' has two inputs and two outputs. 
```matlab
function [quotient,remainder] =  MyRem1(a,b)
```
Calulate the quotient and remainder of a **scalar input** 'a', when divided by the scalar input 'b'.
'quotient' and 'remainder' are **scalar** values.
(b) 'MyRem2' has two inputs and two outputs. 
```matlab
function [quotient,remainder] =  MyRem2(a,b)
```
Calculate the quotient and remainder of each element in an array 'a', when divided by the scalar input 'b'.
'quotient' and 'remainder' have the same dimensions as 'a'.
Must use **MyRem1**

**Solution**
```matlab
% Test MyRem1
[quo_test1,rem_test1] = MyRem1(randi([10 100]), randi([2 10]));

% Test MyRem2
r = randi([1 100]); 
c = randi([1 100]);
mat1 = randi([1 100],r,c);

[quo_test2,rem_test2] = MyRem2(mat1,randi([2 10]));
...
for ii = 1:r*c
    [quotient(ii),remainder(ii)] = MyRem1(a(ii),b);
end

% [quotient, remainder] = MyRem1(a, b);

end
```

## M07058 Push-up/down Zeros (★★★)

Create a function called 'UpDownZeros' that accepts two inputs named 'mat2d' and 'direction' and one output named 'new_mat2d'. 
```matlab
function new_mat2d = UpDownZeros(mat2d,direction)
```
Your function must:
- Move all zeros to the top of the input matrix, preserving the order of non-zero elements, when 'direction' is 'up'
* Move all zeros to the bottom of the input matrix, preserving the order of non-zero elements, when 'direction' is 'down'
Here are some examples:

**Example 1:**

![image](https://user-images.githubusercontent.com/104775472/181605105-bd204719-b982-4fac-9a03-f0bc1fd53299.png)

**Example 2:**

![image](https://user-images.githubusercontent.com/104775472/181605153-12093a76-6838-4d64-a0cf-8988eb4218d3.png)
...
mat2d_test1 = UpDownZeros(mat2d,'up');
mat2d_test2 = UpDownZeros(mat2d,'down');

% please design your function called 'UpDownZeros'

function new_mat2d = UpDownZeros(mat2d,direction)

[r,c] = size(mat2d); % number of rows/columns

new_mat2d = zeros(r, c);

switch direction % switch between direction cases
    
    case 'up'
        for ii=1:c % check each column
            lg_c = mat2d(:, ii) == 0;
            col = [mat2d(lg_c,ii); mat2d(~lg_c,ii)];
            new_mat2d(:,ii) = col;           
        end
...
## M07059 Function: Value replacement using array operation (★★) 

Please use array operation and logical operators in this question. You should not use loop statements.
Create a function named 'RepValues' with one input and one output:
```matlab
function new_array = RepValues(old_array)
```
Your function must:
- replace values of 'old_array' between 0 and 30 (inclusive) with 20
- replace all other values of 'old_array' with 40

**Solution**

```matlab
vec1 = randi([0 60],1,100);
vec2 = randi([0 60],100,1);
mat1 = randi([0 60],100,100);
mat2 = randi([0 60],100,100);

test_vec1 = RepValues(vec1);
test_vec2 = RepValues(vec2);
test_mat1 = RepValues(mat1);
test_mat2 = RepValues(mat2);

% Start your function here

function new_array = RepValues(old_array)

lg_ind = old_array >= 0 & old_array <= 30; % create logical index based on input

new_array = old_array; % initiate new_array
new_array(lg_ind) = 20; % apply 20 to all locations that meet condition
new_array(~lg_ind) = 40; % apply 40 to all locations that don't meet condition

end
```

## M07060 Structure Inspection (★★★★) 
![image](https://user-images.githubusercontent.com/104775472/181605809-33cbb662-383d-454f-8887-054c26274041.png)

The matrix 'force_data' is an n1 x n2 matrix with each data point representing the force acting on the building facade. 
The row and column directions show where on the building each force is acting. 
For example, force_data(2,1) indicates the force value acting on the location of (2,1). You can consider the matrix as locations on the building facade.
The goal of this problem is to find a 2 x 2 sensor location, which is determined by whatever 2 x 2 location has the lowest sum force. 

You need to create a function named 'StrInsp', with one input and two outputs:
```matlab
function [sum_force,sen_loc] = StrInsp(force_data)
```
'force_data' is the force matrix mentioned earlier
'sum_force' is the lowest sum of 2x2 force values in 'force_data'
'sen_loc' is a 4x2 mtrix, containing the best subscript coordinate locations (first column contains row index, second column contains column index) for the sensor in linear indexing order
...
[force_test,loc_test] = StrInsp(force_data_test); % first test of your function

% Create your function below

function [sum_force,sen_loc] = StrInsp(force_data)
    
sum_force = sum(force_data(1:2,1:2),'all');
sen_loc = [1 1; 2 1; 1 2; 2 2];
...
```
## M07061 Extract Characters from a Character Vector (★★)

Create a function named 'ExtractChars' with two inputs and one output:
```matlab
function out_vec = ExtractChars(char_vec,chars)
```
'char_vec' is the input character vector
'chars' a vector of the characters being extracted. Ex: chars = 'aeiou' extracts all vowels from 'char_vec' 
'out_vec' is a vector with only 'chars' characters by preserving their order. 
**Example 1**
```matlab
>> char_vec = 'abbccddeea';
>> chars = 'ab';
>> out_vec = ExtractChars(char_vec,chars)
...
```matlab
char_vec = char(randi([97 122], 1, 100)); % Character vector

% Extract given character combinations
vowels = ExtractChars(char_vec,'aeiou'); 
a_b_c = ExtractChars(char_vec,'abc');
f_j_v = ExtractChars(char_vec,'fjv');

% Write your function below

function out_vec = ExtractChars(char_vec,chars)

num = numel(chars); % Specifiy number of chars input

lg_idx = zeros(1,numel(char_vec),'logical');
for ii = 1:num
    lg_idx = or(lg_idx, char_vec == chars(ii)); % Find every location that contains a letter in chars
end

out_vec = char_vec(lg_idx); % Extract just the letters in chars

end
```

## M07062 Replace Values in an Array (★★)

Write a function, **RepVals**, to find and replace specified values with new values. The function takes 3 inputs, 'array', 'old_vals', and 'new_vals', and produces one output, 'new_array'. 'array' can be a vector or matrix of any size. 
'old_vals' and 'new_vals' are either scalar values or row vectors containing multiple values, but they must be of the same size. The function will replace any occurance of 'old_vals' in 'array' to be 'new_vals' in 'new_array'. 
```matlab
function new_array = RepVals(array, old_val, new_val)
```
For example, if 'old_vals' and 'new_vals' were both scalars:
```matlab
array =
      1 3 4 1 5 2 6
old_vals =
...
new_array =
      8 3 4 8 5 2 6  
```      
Another example if  'old_vals' and 'new_vals' were both vectors:
```matlab
array =
      1 3 4 1 5 2 6
old_vals =
...
new_array =
      8 3 9 8 5 6 6  
```      
As you can see, the values from 'old_vals' are replaced with the corrseponding one in 'new_vals'. The new array has 8s where there were 1s, 9s where there were 4s, and 6s where there were 2s in the old array. 

**Solution**
```matlab
vec = randi([1 100],1,1000);
old_val1 = randi([1 100]);
new_val1 = randi([1 100]);
new_array1 = RepVals(vec, old_val1, new_val1);

mat = randi([1 100],100, 100);
old_val2 = randi([1 100],1,5);
new_val2 = randi([1 100],1,5);
new_array2 = RepVals(mat, old_val2, new_val2);

function new_array = RepVals(array, old_vals, new_vals)

new_array = array;
for ii = 1:numel(old_vals)
    lg = array == old_vals(ii); % make sure to use array, not new_array so that new values are not overwritten
    new_array(lg) = new_vals(ii);
end
end
```
